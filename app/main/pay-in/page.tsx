...
import { searchConfigsPayIn } from '@/utils/searchConfigs';
...
const PayIn: React.FC = () => {
    ...
    const fetchTransactions = async (params: any = {}, page: number = 1) => {
        try {
            // Объединяем фильтры и текущую страницу в один объект
            const queryParams = { ...params, page };
            const encodedData = encodeURIComponent(JSON.stringify(queryParams));
            const response = await fetch(`/api/pay-in/filter?data=${encodedData}`, {
                method: 'GET',
    ...
    return (
        <section className={style.main}>
            <Search searchConfigs={searchConfigsPayIn} onSearch={handleSearch} />
